...
| Agent behaviour or subagents | `docs/guide/agents.md`, AND update all three forms: `src/specsoloist/skills/sp-<name>/SKILL.md`, `.claude/agents/<name>.md`, `.gemini/agents/<name>.md`. Note: Claude and Gemini agents use different tool names and delegation patterns — keep them platform-appropriate (Claude uses `Task` for subagents; Gemini compiles directly due to experimental agent limits). |
